...
class ShopsTable(models.Model):
  ...
  shop_supervisor = models.ForeignKey(EmployeeDetailInformations, on_delete= models.CASCADE)
#   created_at = models.DateTimeField(auto_now_add =True) #never change once the object is created
  updated_at = models.DateTimeField(auto_now =True) # always change when object is updated
...
class ProductAndSupplierAndReceiverTable(models.Model):
  id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, unique = True)
  product_name = models.ForeignKey(ProductTable, on_delete=models.CASCADE)
  product_cost = models.PositiveIntegerField(null =True, blank=True, default=0)
  product_quantity = models.PositiveIntegerField(null =True, blank=True, default=0)
  total_product_cost = models.PositiveIntegerField(null =True, blank=True, default=0)
  amount_to_sold = models.PositiveIntegerField(null =True, blank=True)
  supplier_full_name = models.ForeignKey(ProductAndSupplierTable, on_delete=models.CASCADE)
  shop_info = models.ForeignKey(ShopsTable, on_delete=models.CASCADE)
  product_receiver = models.ForeignKey(EmployeeDetailInformations, on_delete=models.CASCADE)
  created_at = models.DateTimeField(auto_now_add =True) #never change once the object is created
  updated_at = models.DateTimeField(auto_now =True) # always change when object is updated
...
class  productSoldInCash(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, unique = True)
    product_name = models.ForeignKey(ProductAndSupplierAndReceiverTable, on_delete=models.SET_NULL, null=True)
    ...
    supervisor = models.ForeignKey(EmployeeDetailInformations, on_delete=models.SET_NULL, null=True)
